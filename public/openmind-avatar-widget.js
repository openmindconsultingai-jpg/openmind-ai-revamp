(() => {
  ...
  const initialLanguage = languageOrder.some((item) => item.code === (script?.dataset.language || locale))
    ? (script?.dataset.language || locale)
    : 'pl';
  ...
  function languageOptionsHtml() {
    return languageOrder.map((language) => {
      ...
      return '<option value="' + escapeAttr(language.code) + '"' + selected + '>' + escapeAttr(language.label) + '</option>';
    }).join('');
  }

  function avatarUrlWithLanguage(rawUrl) {
    try {
      const url = new URL(rawUrl, window.location.href);
      url.searchParams.set('orientation', url.searchParams.get('orientation') || 'horizontal');
      url.searchParams.set('language', state.language);
      url.searchParams.set('locale', state.language);
      url.searchParams.set('lang', state.language);
      if (state.avatarRefresh) url.searchParams.set('_om_reload', String(state.avatarRefresh));
      return url.toString();
    } catch {
      return rawUrl;
    }
  ...
  function syncLanguageControls() {
    ui.widget.querySelectorAll('[data-language-control]').forEach((select) => {
      select.value = state.language;
    });
  }

  function setLanguage(language) {
    state.language = languageOrder.some((item) => item.code === language) ? language : 'pl';
    syncLanguageControls();
    reconnectAvatar();
  }

  function reconnectAvatar() {
    state.avatarRefresh = Date.now();
    renderAvatarEmbed(state.config || fallbackConfig(), { force: true });
  }
  ...
  function buildWidget() {
    ...
    widget.innerHTML = `
      ...
      <div class="om-voice-toolbar">
        <select data-role="language-select" data-language-control aria-label="Język avatara">${languageOptionsHtml()}</select>
        <button type="button" data-role="reconnect-avatar">Połącz ponownie</button>
      </div>
  ...
  const languageSelect = ui.widget.querySelector('[data-role="language-select"]');
  const reconnectButton = ui.widget.querySelector('[data-role="reconnect-avatar"]');

  function renderAvatarEmbed(clientConfig, options = {}) {
    const embed = clientConfig?.avatar_embed;
    if (!embed?.url) return false;
    const video = ui.widget.querySelector('.om-avatar-video');
    if (!video || (video.dataset.embedInstalled === 'true' && !options.force)) return true;
    video.dataset.embedInstalled = 'true';
    const src = avatarUrlWithLanguage(embed.url);
    const allow = embed.allow || 'microphone; camera; autoplay; clipboard-write; encrypted-media';
    video.innerHTML = `<iframe class="om-avatar-iframe" src="${escapeAttr(src)}" allow="${escapeAttr(allow)}" title="${escapeAttr(embed.title || 'LiveAvatar Embed')}" referrerpolicy="origin" allowfullscreen></iframe><div class="om-heygen-language-cover" aria-label="Wybór języka avatara"><select data-role="iframe-language-select" data-language-control aria-label="Język avatara">${languageOptionsHtml()}</select></div>`;
  ...
  languageSelect?.addEventListener('change', () => setLanguage(languageSelect.value || 'pl'));
  reconnectButton?.addEventListener('click', reconnectAvatar);
  ui.widget.addEventListener('change', (event) => {
    if (event.target?.matches?.('[data-role="iframe-language-select"]')) {
      setLanguage(event.target.value || 'pl');
    }
  });
