...
- Weryfikacja wpięta w `send-contact-email`, `send-booking-confirmation`, `openai-chat` oraz `ai-advisor`.
- Model sesji czatu: przy pierwszej wiadomości nowej konwersacji frontend pobiera token reCAPTCHA i wysyła go razem z wiadomością. Backend weryfikuje go u Google i zwraca podpisany „paszport sesji” (krótki podpisany token HMAC z czasem ważności, np. 2 h, oparty o sekret backendu). Kolejne wiadomości tej samej rozmowy wysyłają już tylko paszport — bez odpytywania Google. Wygaśnięcie paszportu lub start nowej konwersacji (odświeżenie strony, przycisk „nowa rozmowa”, wyczyszczenie historii) uruchamia weryfikację od nowa.
- Niski score na starcie rozmowy → uprzejmy komunikat zamiast odpowiedzi AI, bez wywołania modelu.
